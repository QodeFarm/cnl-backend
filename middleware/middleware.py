...
def SubdomainFromMstcnl():
    domains = list(License.objects.using('mstcnl').values_list('domain', flat=True))
    databases = list(License.objects.using('mstcnl').values_list('database_name', flat=True)) 

    DOMAIN_DATABASE_MAPPING = dict(zip(domains, databases))
...
class DatabaseMiddleware(MiddlewareMixin):
    def process_request(self, request):
        ...
        db_name = sub_dom_dict.get(client_domain, "devcnl")  # Default to devcnl if not found

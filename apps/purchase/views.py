...
class PurchaseOrderViewSet(APIView):
    ...
    # def get_purchases_by_vendor(self, request):
    #     """Fetches total purchase amount grouped by vendor."""
    #     logger.info("Retrieving purchases by vendor data")

    #     page, limit = self.get_pagination_params(request)

    #     # Vendor-wise total purchase amount calculation
    #     queryset = PurchaseOrders.objects.values(vendor=F('vendor_id__name')).annotate(
    #         total_purchases=Sum('total_amount', output_field=models.DecimalField())  

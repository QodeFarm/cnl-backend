...
from apps.users.views import UserUpdateView, RolePermissionsCreateView, CustomUserActivationViewSet, CustomUserCreateViewSet, RoleViewSet, ModulesViewSet, ActionsViewSet, ModuleSectionsViewSet, RolePermissionsViewSet, SendPasswordResetEmailView, UserChangePasswordView, UserPasswordResetView, UserTimeRestrictionsViewSet, UserAllowedWeekdaysViewSet, UserLoginView, UserRoleViewSet
...
urlpatterns = [
    ...
    path('user/<uuid:user_id>/', UserUpdateView.as_view(), name='user-detail'),# ==>get, update, delete this opertion taken care by user API 
    path('user/', UserUpdateView.as_view(), name='user-list')# get all user API

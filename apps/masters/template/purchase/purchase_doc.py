...
def purchase_data(pk, document_type, format_value=None):
    ...
    total_amt = total_qty = total_txbl_amt = cgst = sgst = igst = total_cgst = total_sgst = total_igst = cessAmt = total_disc_amt = party_old_balance = net_value = 0.0
    ...
    rounded_total = round(total_bill_amt)  # This rounds to nearest rupee
    round_off = rounded_total - total_bill_amt  # This will be positive or negative
    
    # If you want to always round down (floor) like your example 54.13 -> 54.00
    # Use this instead:
    # rounded_total = int(total_bill_amt)  # This truncates decimal part
    # round_off = rounded_total - total_bill_amt  # This will be negative (e.g., -0.13)
    
    # If you want to always round up (ceil):
    # rounded_total = int(total_bill_amt) + 1 if total_bill_amt % 1 > 0 else int(total_bill_amt)
    # round_off = rounded_total - total_bill_amt  # This will be positive (e.g., 0.87)
    
    # Display round_off with proper sign
    round_off_display = round(round_off, 2)
    # For display: if round_off is negative, show as -0.13; if positive, show as 0.87
    # ============================================
    ...
    return {
            ...
            # Add round-off fields
            'rounded_total' : rounded_total,  # The rounded amount
            'round_off' : round_off_display,   # The difference (can be negative or positive)
...
def purchase_doc(
        ...
        round_off=None,           # Add this parameter
        rounded_total=None,       # Add this parameter
        print_config=None
        ):
    ...
    # Pass round-off values to narration_and_total

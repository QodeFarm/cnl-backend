...
def purchase_data(pk, document_type):
      #Companies Details
        ...
        vendor = serializer(purchase_order).data.get('vendor')      
        billing_address =  serializer(purchase_order).data.get('billing_address')
        shipping_address =  serializer(purchase_order).data.get('shipping_address')
        order_no =  serializer(purchase_order).data.get('order_no')
        order_date =  serializer(purchase_order).data.get('order_date')
        tax_type = serializer(purchase_order).data.get('tax')
        ...
        total_sub_amt = total_amt - total_disc_amt

        total_bill_amt = total_sub_amt + total_txbl_amt
...
def purchase_order_return(pk, document_type):
      #Companies Details
        company = Companies.objects.all()
        serializer = CompaniesSerializer(company, many=True)
        comp_data = serializer.data
        ...
        purchase_return_order = get_object_or_404(model, pk=pk)
        ...
        vendor = serializer(purchase_return_order).data.get('vendor')  

        billing_address =  serializer(purchase_return_order).data.get('billing_address')
        shipping_address =  serializer(purchase_return_order).data.get('shipping_address')
        order_no =  serializer(purchase_return_order).data.get('return_no')
        order_date =  serializer(purchase_return_order).data.get('return_date')
        tax_type = serializer(purchase_return_order).data.get('tax')

        # print(vendor.get('name'), billing_address, shipping_address, order_no, order_date, tax_type)
        # Retrieve related data
        items_data = get_related_data(item_model ,items_serializer  , item_model_pk, pk)
        print(items_data)
        total_amt = total_qty = total_txbl_amt = total_disc_amt = round_0ff = party_old_balance = net_value = 0.0
        ...
        # # attachments_data = get_related_data(related_model, related_serializer, order_id, pk)
        shipments_data = get_related_data(related_model, related_serializer, related_filter_field, pk)
        shipments_data = shipments_data[0] if len(shipments_data)>0 else {}

...
from apps.production.filters import BOMFilter, BOMReportFilter, BillOfMaterialsFilter, MachineFilter, MachineUtilizationReportFilter, MaterialFilter, ProductionCostReportFilter, ProductionStatusFilter, ProductionSummaryReportFilter, RawMaterialConsumptionReportFilter, WorkOrderFilter
...
class WorkOrderAPIView(APIView):
    ...
    # def apply_filters(self, request, queryset, model_class):
    #     """Applies filters dynamically based on the model class."""
    #     # Map model classes to their respective filter classes
    #     filter_mapping = {
    #         WorkOrder: WorkOrderFilter,
    #         BillOfMaterials: BillOfMaterialsFilter
    #     }
    #     # Get the correct filter class for the model
    #     filter_class = filter_mapping.get(model_class, None)
        
    #     if filter_class:
    #         filterset = filter_class(request.GET, queryset=queryset)
    #         if filterset.is_valid():
    #             return filterset.qs
    #     return queryset  # If no filter class found, return unfiltered queryset
    ...
    def get_production_cost_report(self, request):
        ...
        # queryset = WorkOrder.objects.values('product_id__name').annotate(
        #         material_cost=Coalesce(Sum('product_id__billofmaterials__total_cost'), Value(0), output_field=DecimalField()),
        #         labor_cost=Coalesce(Sum('productionworker__hours_worked') * 10, Value(0), output_field=DecimalField()),
        #         machine_cost=Coalesce(Sum('workordermachine__machine_id') * 5, Value(0), output_field=DecimalField()),
        #         total_production_cost=Coalesce(
        #             Sum('product_id__billofmaterials__total_cost') + (Sum('productionworker__hours_worked') * 10) + (Sum('workordermachine__machine_id') * 5),
        #             Value(0), output_field=DecimalField()
        #         )
        #     ).order_by('-total_production_cost')
    ...
    def get_machine_utilization_report(self, request):
        ...
        #     ),
        #     total_work_orders=Count('work_order_id', distinct=True),
        #     avg_usage_per_work_order=ExpressionWrapper(
        #         F('total_usage_hours') / Coalesce(F('total_work_orders'), Value(1)),  # Prevent division by zero
        #         output_field=DecimalField()
        #     ),
        #     downtime_hours=ExpressionWrapper(
        #         Value(168) - F('total_usage_hours'),  # Assuming a 168-hour work week
        #         output_field=DecimalField()
        #     )
